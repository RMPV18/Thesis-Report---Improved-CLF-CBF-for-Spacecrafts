...
Where \(\sigma(V_{C_{j|i}}(\xVec))\) is a sigmoide function responsible to the transition between the \glsxtrshort{CLF} \(V_{C_{j|i}}\) and \(V_{N_{i}}\), \(\zeta \in \mathbb{R}_{\geq 0}\) is its slope and \(\mathcal{S}_j\) the just one time activation set of the respective \glsxtrshort{CLF} \(V_j\). 
