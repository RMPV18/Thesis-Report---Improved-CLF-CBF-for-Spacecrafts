...
\glsxtrshort{MPC-L} shows the shortest path and less input consumed, however since the optimization is done based just on the last state from the horizon resulting in a reaching time close to the end of the horizon. This \glsxtrshort{MPC} version targets the end of the horzion with minimal consume, but if the horizon is too short, assuming the same weight on the last state, it can lead to really high control inputs to be as close to \txtref at the end of it. Meanwhile, the \glsxtrshort{A-CLF-S} optimization don't suffer from the same proble due to its baseline framework, more precisely, the static behaviour imposed by the \(\gamma\) function, i.e, the total input cost variation is impacted essentially by its the path which is effectively optimized via \(\alpha_d\) optimization by the respective cost function~\ref{eq:Cost_Function_Alpha_last_state}.
 ...
 Computationally wise, both \glsxtrshort{MPC} generally show similar times.
