...
The function \(SPS\)~\ref{eq:CLF-CBF_RK4_Propagation} and \(DGF\)~\ref{eq:discrete_gradient_function} parameters used in these experiments are given by:
 ...
 \begin{xltabular}{\textwidth}{ccccc}
   ...
   \midrule
     3          &  0.01        & 0.005        &  \ref{eq:Cost_Function_Alpha}   &   \ref{subsubsec:Unicyle_CLF-CBF_Experiment_Setup} or \ref{subsubsec:Orbital_CLF-CBF_Experiment_Setup}\\
   \midrule
   \end{xltabular}
 \egroup
...
Following the results obtained previously, it is compared the \glsxtrshort{MPC} using the objective function \ref{eq:Cost_Function_Alpha} (\glsxtrshort{MPC-F}), with a \glsxtrshort{MPC} with the cost defined according to \ref{eq:Cost_Function_Alpha_last_state} (\glsxtrshort{MPC-L}) like \glsxtrshort{A-CLF-S}~\ref{subsubsec:A-CLF-S_parameters}.\\
