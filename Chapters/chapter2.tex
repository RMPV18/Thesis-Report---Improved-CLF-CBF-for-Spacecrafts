...
Where the \(step\) is always equal. That being  said, the phase 1 algorithm: 
...
It is characterized to a discrete behaviour as the search for the point is through steps along \(\mathbf{v} = \bigl[\begin{smallmatrix} \mathbf{v}_1 \\ \mathbf{v}_2 \end{smallmatrix}\bigr]\) and its perpendicular direction \(\mathbf{vp} = \bigl[\begin{smallmatrix} -\mathbf{v}_2 \\ \mathbf{v}_1 \end{smallmatrix}\bigr]\). 
Given \(p = \mathbf{O}\xVec_{END}\) and \(v\) returned by \(NCDV\)~\ref{alg:Natural_Convergence_Direction_Vector}, the first order \glsxtrshort{CBF} \(h0\), arbirtrary steps sizes, \(step\) and \(stepP\), along \(v\) and \(vp\) respectively and a maximum of iterarions while searching through the respective directions. 
  ...
  \begin{algorithm}
    ...
    $(p_l, q_l) \gets (p, q)$ \Comment*[r]{Initializing the left pait of points}
     
    $(p_r, q_r) \gets (p, q)$ \Comment*[r]{Initializing the right pait of points} 
...
Depending how small the steps are and how big is the total number of searching iteration steps along \(v\) and \(vp\) (and their inverses \(-v\) and \(-vp\)), the more robust the algoritm is.\\
  ...
  From \(FNLIs\) obtains both intersection points between the layer and the unsafe set according to the \glsxtrshort{CBF} \(h0\).\\
  ...
  This algorithm is responsible to obtain one of the layer intersection for a chosen side according to the direction stipulated \(v\).\\
 ...
 It's function is to obtain the intersection point between a layer and boundary. It starts from a point already inside boundary and it is propagated through \(v\) direction. \\
...
\begin{equation}
    \begin{array}{ll}
        g_{\xVec_0}(\alpha) &= g(\alpha, \xVec_0 = cte)  \\   
        &= \xVec_{N+1}(\alpha,\xVec_0)^{\top}\mathbf{Q}\xVec_{N+1}(\alpha,\xVec_0) + \sum_{k=0}^{N}  \uVec_k(\alpha,\xVec_0)^{\top}\mathbf{R}\uVec_k(\alpha,\xVec_0) \\
        &= \xVec_{N+1}(\alpha,\xVec_0)^{\top}\mathbf{Q}\xVec_{N+1}(\alpha,\xVec_0) +  \mathbf{U}(\alpha,\xVec_0)^{\top}\tilde{\mathbf{R}}\mathbf{U}(\alpha,\xVec_0)
    \end{array}
...
\begin{equation}
    \begin{array}{ll}
        g_{\xVec_0}(\alpha) &= g(\alpha, \xVec_0 = cte)  \\   
        &= \sum_{k=0}^{N} \xVec_{k+1}(\alpha,\xVec_0)^{\top}\mathbf{Q}_i\xVec_{k+1}(\alpha,\xVec_0) +  \uVec_k(\alpha,\xVec_0)^{\top}\mathbf{R}\uVec_k(\alpha,\xVec_0)
    \end{array}
...
Finally, with \glsxtrshort{A-JO}~\ref{subsec:Just_Optimized_Algorithm}, using the cost function \ref{eq:Cost_Function_Alpha_last_state} adapt the \glsxtrshort{CBF} slope \(\alpha\) to the new low and high order \glsxtrshort{CLF}s and improve possibly even more the state convergence to the final equilibrium point.\\


\textbf{\underline{Three Waypoints Version}}
\label{subsubsubsec:CLFs_Summed_Algorithm_three_waypoints}\\

If the unsafe set is restricted by a polytopic fit~\ref{eq:Polytope_fit} and the contour point obtaining algorithm is \ref{} then it is returned two equilibrium points. Those points are approximately two vertices of the polytope of a same edge or just one vertice if they are too close.  It keeps having a current point a \glsxtrshort{CLF} is trying to converge but the next one don't necessarily is the desired final equilibrium point \(\bar{\xVec}_F\) but the other computed point. 

Then the first \glsxtrshort{CLF} are defined as:
...
\begin{align}
    V_0(\xVec) = 
    \begin{cases}
        V_{0_i}(\xVec) \quad \xVec \in \mathcal{S}_i \qquad i \in {1,2}\\
        V_{0_3}(\xVec) = V_{F_0}(\xVec) \quad \xVec \in \bigcup_{i = 1}^2 \bar{\mathcal{S}}_i
    \end{cases}
