...
In this chapter, it is presented a low-effort \glsxtrlong{CLF-CBF} based decision algorithm, actively imposing safety and convergence to an desired equilibrium point, with its computation speed potentialized by its closed-form solution. Futhermore, to be able to deal with high-order systems the nominal framework of the \glsxtrshort{CLF-CBF} is generalized via Backstepping. Adding to the \glsxtrshort{CLF-CBF} method, in order to better capture the shapes of the unsafe sets, then it is presented a strategy through smooth approximation of logical operations to merge multiple \glsxtrshort{CBF}.   \par

The objective of this thesis is driving a system to follow an trajectory as close as possible to an optimal one with minimal effort through \glsxtrshort{CLF-CBF} based strategies encompassing the optimization of a exclusive single parameter and instructing the system to converge to another equilibrium point on the boundary of the safe set.

%defing the circles function of the circles 
\def\firstcircle{( 90:1.5) circle (2cm)} %(Direção (graus), distancia nessa direção (Coordenadas Polares)) (raio do circulo)
...
\begin{tikzpicture}[blend group=screen]
  \fill[red!90!black]   \firstcircle  node[ white!220] {\textbf{CLF-CBF}}; 
  \fill[green!80!black] \secondcircle node[ white!220] {\textbf{Optimization \hspace{1em}} };
  \fill[blue!90!black]  \thirdcircle  node[ white!220] {\hspace{1em}\textbf{Stability Change}};
  \draw (330:1.5) circle (1pt) node[below, white!220]  {\hspace{1em}\textbf{to a Contour Point}}; 


%   \fill[red!90!black]   ( 90:.6) circle (1) node[below, white!220] {\glsxtrshort{CLF-CBF}};
%   \fill[green!80!black] (210:.6) circle (1) node[above, white!220] {OPtimization};
%   \fill[blue!90!black] (330:.6) circle (1) {Stability Change};
\end{tikzpicture}
...
The scope of this dissertation is not restricted just to the \glsxtrshort{CLF-CBF}. However, the achieved control techniques, in addition to the traditional \glsxtrshort{CLF-CBF} (red), are based on it, with the possibility of adopting the optimization (yellow) or stability change to a computed point (pink) strategy , even both together (gray).
...
%   \fill[red!90!black]   \firstcircle  node[ white!220] {\textbf{CLF-CBF}}; 
%   \fill[green!80!black] \secondcircle;% node[ white!220] {\textbf{Optimization \hspace{1em}} };
%   \fill[blue!90!black]  \thirdcircle;%  node[ white!220] {\hspace{1em}\textbf{Contour Point}};


% \begin{scope}
%         \begin{scope}[even odd rule]% first circle without the second
%             \clip \firstcircle (-4,-4) rectangle (4,4);
%         \fill[white!100] \secondcircle  ;
%         \end{scope}
...
%         \begin{scope}[even odd rule]% first circle without the second
%             \clip \firstcircle (-4,-4) rectangle (4,4);
%         \fill[white] \thirdcircle  node[blue!90!black] {\hspace{1em}\textbf{Contour Point}};
%         \end{scope}
        
% \end{scope}

% \draw[green!80!black] \secondcircle   node[green!80!black] {\textbf{Optimization \hspace{1em}} };
% \draw[blue!90!black] \thirdcircle   node[blue!90!black] {\hspace{1em}\textbf{Contour Point}};
...
\begin{scope}
        \begin{scope}
            \clip \firstcircle;
        \fill[green!80!black] \secondcircle  ;%Esse verde mistura com o vermelho das CLF-CBF
        \end{scope}
...
\begin{scope}
        \begin{scope}
            \clip \firstcircle;
        \fill[blue!90!black] \thirdcircle  ;
        \end{scope}
        
\end{scope}

\draw[green!80!black] \secondcircle   node[green!80!black] {\textbf{Optimization \hspace{1em}} };
\draw[blue!90!black] \thirdcircle   node[blue!90!black] {\hspace{1em}\textbf{Stability Change}};
\draw (330:1.5) circle (1pt) node[below, blue!90!black]  {\hspace{1em}\textbf{to a Contour Point}}; 
...
%tiktx figura cortadas com partes que intersetam as CLF-CBF  ou que o restante em branco... e defenindo cada secção nas respetivas partes resultantes que englobam, se não der para meter la no nome das secçoes sem ficar emcavalitado, meter mais uma figura com duas subfiguras cada uma com a parte da respetiva secção.


\section{Chapter Structure}
\label{ch1_sec:Chapter_Structure} 

This chapter starts presenting the mathematical formulation of the \textbf{Optimal Control Problem} (section \ref{ch1_sec:Chapter_Structure}) which results are pretended to approximate with the developed techniques presented in this chapter. \par 
The proposed methods methodology starts with the \textbf{\glsxtrshort{CLF-CBF}} (section \ref{ch1_sec:clf_cbf}) which they are based on. This section is structured in the following manner: starts introducing the \glsxtrshort{CLF-CBF} nominal formulation, the Relative Degree One Formulation (subsection \ref{sub:formulation}), more specifically, presenting a \glsxtrshort{CLF} (subsubsection \ref{subsub:control_lyapunov_function}), a \glsxtrshort{CBF} (subsubsection \ref{subsub:control_barrier_function}) and finally the \glsxtrshort{QP}  formulation (subsubsection \ref{subsub:quadratic_program_formulation}). Followed by the Backstepping (subsection \ref{sub:backstepping}), described as the high-order formulation, then by the Closed-Form solution of the previous problems from the section (subsection \ref{sub:closed_form}) and finally by the \glsxtrshort{CBF} Unifying Method (subsection \ref{sub:unifying_CBF}), for the first (subsubsection \ref{subsub:uniCBF_firstOrder_system})  and high order (subsubsection \ref{subsub:uniCBF_secondOrder_system}) formulation of the \glsxtrshort{CBF}.\par
The first proposed technique it follows the \textbf{Single Parameter Optimization} strategy (section \ref{sec:Single_Parameter_Optmization}) which stands on a cost function and its gradient, discretized (subsection \ref{subsec:Gradient_Cost_Function_Discretization}) if it is not possible to obtain the exact equation. In that section it is presented two optimization techniques, namely, Gradient Descent (subsection \ref{subsec:Gradient_Descent}) and a Binary Search variation (subsection \ref{subsec:Binary_Search}).\par 
The other strategy depends on the \textbf{Contour Point} (section \ref{sec:Constraint_Contour_Point}), which proposed obtaining method, is composed by two pahses. The first one responsible to find a Line of Intersection with the constrained set (subsection \ref{subsec:}) adopting a tatic based on the system evolution simulation, making use of Runge-Kutta methods (subsubsection \ref{subsub:Runge-Kutta_Methods}) in the State Propagation Simulation Algorithm (subsubsection \ref{subsub:State_Propagation_Simulation_Algorithm}). The first phase is consummated by the Natural Convergence Line Algorithm (subsubsection \ref{subsubsec:Intersection_Line}). The second phase corresponds to bring to completion the obtaing method by the Farest Boundary Point from the Intersection Line (subsection \ref{subsec:Constraint_Contour_Point_Algorithm}).\par
This chapter concludes with the formulation of the \glsxtrshort{CLF-CBF} based \textbf{Proposed Decision Algorithms} (section \ref{sec:Proposed_Decision_Algorithms}), given the Single Parameter Optmization Strategy (subsection \ref{subsec:Just_Optimized_Algorithm}) and the Stability Change to a Contour Point (subsection \ref{subsec:CLFs_Summed_Algorithm}), both encompassing a variety of techniques given the respective srategies and \glsxtrshort{CLF-CBF} formulations.


%A_JO vai passar a secção das other cost functions para o single parameter optimization, para passar a referir a uma tecnica que usa otimização como A_O, e se usar todo o horizonte na cost function então A_{O_F}, e se for só o útimo estado então A_{O_L} (depois dizer que no caso de usar a CLF o ultimo estado não importa, basta o input embora os resultados sejam parecidos especialmente quanto..., enquanto se for a CLF-CBF dá igual eu acho)
%Deixar a regra clara para os nomes para quem ler ser capaz de destinguir.
...
If those conditions, \eqref{eq:LF_derivative_condition} are verified it means the Lyapunov function \(V\) converges to zero while the system converges to the equilibrium point, hence the system is asymptotically (lyapunov) stable, which is the notion of stability considered along this thesis.\par
...
In order to generalize to controllable dinamical systems, it is introduced \glsxtrshort{CLF}, modeled just like the typical the lyapunov functions~\eqref{eq:LF_condition} but able through the use of the given control inputs affect its derivative in order to achieve the stability conditions~\eqref{eq:LF_derivative_condition}. That beeing said, it is pretended to the \glsxtrshort{CLF} function \( V\mathbf(\mathbf{x}) \) to be continuous and differentiable:
...
While the \glsxtrshort{CLF} imposes an "acttracting" behaviour to the system~\eqref{eq:Nonlinear-First-Order-Closed-Loop-System}, the \glsxtrshort{CBF} a "repelling" one. Having the objective of guarantee safety to system, is aimed to keep the state inside a safe set \(\mathcal{C}\) (forward invariant\footnote{A set \(\mathcal{S}\) is said forward invariant if the initial system state \( x_0 \in \mathcal{S}\) and so \(x(t) \in \mathcal{S} \hspace{0.25em} \forall t \geq 0\) }) defined accordingly the problem needs (e.g. in the context autonomous vehicles, the outside space occupied by an obstacle).\par 
...
\begin{itemize}
    ...
    %\item If it's a higher-order (than second) system, then, assuming it's a third-order system, the solution obtained in \eqref{eq:CLF-CBF-QP-relaxed-1Backstep} is the "new \(k_0\)" and the third-order solution tries to converge to the second-order one (while this one is converging to the first-order solution)
\end{itemize}
\end{tcolorbox} 
...
\begin{description}
    ...
    Therefore the higher \(b\), the safer the system~\eqref{eq:NL-CL-System-0Backstep} but possibly too constrained by the new \glsxtrshort{CBF} \(h(\xVec)\) \eqref{eq:Max_smoothFunc_tighted} (figure 1 from \cite{molnar2023composing}). 
\end{description}
...
Given a \glsxtrshort{CLF} \(V_0(\xVec): \) and multiple \glsxtrshort{CBF} \(h0_i(\xVec) \quad \forall i \in \mathcal{I}\) combined into a \glsxtrshort{CBF} \(h_0(\xVec)\) via \eqref{eq:Min_smoothFunc_tighted} (intersection) or \eqref{eq:Max_smoothFunc_tighted} (union) (they can be combined using both operations), through the \glsxtrshort{QP}~\eqref{eq:CLF-CBF-QP-relaxed-0Backstep} (or its closed-form solution~\eqref{eq:closed-form_controller}) is obtained a differentiable and locally Lipschitz continuous theoretical controller \(k_0(\xVec): \mathbb{R}^n \to \mathbb{R}\). \\

In order to the second-order system~\eqref{eq:HH-NL-CL-System} dynamic converge to \(k_0(\xVec)\) imposed dynamic, given multiple \glsxtrshort{CBF} \(h_i(\xVec) \quad \forall i \in \mathcal{I} \):
...
Having \glsxtrshort{CBF} \(h\), the second-order solution, a differentiable and locally Lipschitz continuous controller \(k(\xVec, \mathbf{\xi}): \mathbb{R}^n \times \mathbb{R}^p \to \mathbb{R}\) is obtained using the \glsxtrshort{QP}~\eqref{eq:CLF-CBF-QP-relaxed-1Backstep} (or its closed-form solution~\eqref{eq:closed-form_controller}). \\
...
\SetKwComment{Comment}{/* }{ */}
  \begin{algorithm}
    ...
    \While{$i \leq nS -1$}{
        ...
        $grad_i \gets \nabla g(x + (-1)^{i}  \times devN_i, (-1)^i \times dev)$ \Comment*[r]{Equation \eqref{eq:Gradient_Discretization}}
        $i\gets i + 1$\;    
        }
...
\SetKwComment{Comment}{/* }{ */}
  \begin{algorithm}
    \KwData{$xI$, $a$}
    \KwResult{$sort(xI,xS)$}
    $xS \gets xI - a \nabla g(x_I)$    \Comment*[r]{Equation \eqref{eq:Binary_Search_Phase1_gradDesc}}
    \While{$\nabla g(x_I) \times \nabla g(xS) > 0$}{
        $xI\gets xS$\;
        $xS \gets xI - a \nabla g(x_I)$\;
    }
...
\SetKwComment{Comment}{/* }{ */}
  \begin{algorithm}
    ...
    \While{$i < tI$ \textbf{AND} Conditions$^*$}{   
        ...
        \eIf{$\nabla g(xMid) > 0$}{
            $xS\gets xMid$\;
            $w \gets w - sig_{-}(w)$  \Comment*[r]{Sigmoide function \eqref{eq:binsea_weight_update_function}}
        }{
            $xI\gets xMid$\;
            $w \gets w + sig_{+}(w)$\;
        }
...
Having the objective of global asymptotically  stability of a system, the system converges a desired equilibrium point \(\bar{\xVec}_F\) but kept outside a bounded unsafe set \(\mathcal{B} \subset \mathbb{R}^n\). Depending on certain decision algorithms, such as \eqref{eq:CLF-CBF-QP-relaxed} which uses \glsxtrshort{CLF-CBF}, by pursuing another equilibrium point \(\bar{\xVec}_I\) first, the path taken by the system's states \(\xVec \in \mathbb{R}^n\) until \(\bar{\xVec}_F\) could be oriented in order to be a better fit for problem demands, specially to deal more efficiently with the implications of an unsafe set \(\mathcal{B}\), e.g., following a safer and shorter path.\\
...
\label{sub:State_Propagation_Simulation}
...
The \underline{\glsxtrshort{RK} 4th order} is defined by \eqref{eq:Explicit-RK} and its tableau:
...
Where the intermediate derivatives \(k_i\) functions keep the same equation \eqref{eq:Derivatives_Explicit-RK} and are equal for both (low and high order methods) reducing some computing time. The local truncature error estimations ig given by:
...
Finding a new equilibrium point \(\bar{\xVec_I}\) requires simulating state convergence. Aiming to simulate a system evolution using a decision algorithm, given the ordinary differential states dynamics equations \(\dot{\xVec} = f(\xVec,\uVec)\) propagated via a runge-kutta method~\ref{subsub:Runge-Kutta_Methods}:
  ...
  \begin{algorithm}
    ...
    \While{$i < tI$ \textbf{AND} Conditions$^*$}{   
        ...
        $\xVec_{i+1} \gets integration^*(f, \xVec_i, \uVec_i, \Delta t)$ \Comment*[r]{\small Euler~\eqref{eq:Euler_Method},  \glsxtrshort{RK} 4~\ref{subsubsub:RK_Explicit} or DP~\ref{subsubsub:RK_Adaptive}...}
        $i \gets i +1$ \;
    }
...
\subsubsection{Natural Convergence Line}
\label{subsubsec:Intersection_Line}
...
%   OR
  ...
  \begin{algorithm}
    ...
    \caption{Natural Convergence Line (\(NCDL\))} \label{alg:Natural_Convergence_Direction_Line}
  \end{algorithm}
...
Both algorithms are responsible to obtain the farest point (or points \ref{sssec:Constraint_Tangent_Point_Any}), restricted to the two-dimensional Euclidean Space, given the perpendicular direction of \(dir\), obtained using algorithm \ref{alg:Natural_Convergence_Direction_Line}.\par
...
Given \(\mathbf{p} = \mathbf{O}\xVec_{END}\) and \(\mathbf{v}\) returned by \(NCDL\)~\ref{alg:Natural_Convergence_Direction_Line}, the first order \glsxtrshort{CBF} \(h0\), arbitrary  steps sizes, \(step\) and \(stepP\) along \(\mathbf{v}\) and \(\mathbf{vp}\) respectively and a maximum of iterations while searching through the respective directions. 
...
Based on single parameter optimization~\ref{sec:Single_Parameter_Optmization}, the proposed technique is centered on the slope \(\alpha_d\) of the \glsxtrshort{CBF} constraint function \(\alpha:\mathbb{R} \to \mathbb{R}\) (\glsxtrshort{CLF-CBF} \glsxtrshort{QP}~\eqref{eq:CLF-CBF-QP-relaxed}) as an optimization variable:
...
Because of the n states and inputs continuous differential solution (\(\xVec(t) and \uVec(t)\)), potentialized by the conditioned control input values given by CLF-CBF closed-form controller~\eqref{eq:closed-form_controller}, the gradient \(\nabla g_{\xVec_0}(\alpha_d)\) exact equation is difficult to have it, hence is required a discretization of it~\ref{subsec:Gradient_Cost_Function_Discretization}. Which leds to the exact function of \(g_{\xVec_0}(\alpha_d)\) which is not known as \(\uVec_k\) and \(\xVec_{k+1}\) could not be obtained directly.\par 
Digitally, through the \(SPS\) algorithm \ref{alg:Discrete_Gradient_Function} is computed the approximated states \(\mathbf{X}(\alpha_d,\xVec_0)\) and inputs \(\mathbf{U}(\alpha_d,\xVec_0)\) along the horizon. Like it was said, \(\uVec_k\) is obtained based on a closed-form \glsxtrshort{CLF-CBF} controller~\ref{sub:closed_form}, governed by a \glsxtrshort{CLF} \(V\) and a \glsxtrshort{CBF} \(h\). Meanwhile, given the control action \(\uVec_k\), \(\xVec_{k+1}\) is propagated by an explicit \glsxtrshort{RK}\ref{subsubsub:RK_Explicit} in particular defined by the fourth order \glsxtrshort{RK} Butcher Tableau\eqref{eq:RK4_Tableu}, more accurate than the Euler method~\eqref{eq:Euler_Method} and faster than the \glsxtrshort{DP}~\eqref{eq:Dormand-Prince_Tableu}. So given the system dynamics equation \(f:\mathbb{R}^n \times \mathbb{R}^m \to \mathbb{R}^n\) and integration time interval \(\Delta t\):
...
Afterwards, having the optimized \(\alpha_d\), it is used the typical \glsxtrshort{CLF-CBF} framework, more precisely the closed-form controller \eqref{eq:closed-form_controller} as decision algorithm to speed up.
...
\begin{equation}
    \begin{array}{l}
        [dir, \hspace{0.1em} \xVec_{int}] = NCDL(f, \xVec_0, N, \Delta t, V, h) \hspace{0.2em} \begin{bmatrix*}[l] decision^* \gets \text{\glsxtrshort{CLF-CBF}~\ref{sub:closed_form} } \\ integration^* \gets \text{\glsxtrshort{RK}4~\eqref{eq:RK4_Tableu}} \end{bmatrix*}  \\
...
Where \(NCDL\) (Natural Convergence Line) is algorithm~\ref{alg:Natural_Convergence_Direction_Line}, and \(\mathbf{O} \in \mathbb{R}^{2 \times n}\) the constrained states observer matrix. Regarding the choice of \glsxtrshort{RK}4 over other methods is because as it was said before, since it is more accurate than the Euler method~\eqref{eq:Euler_Method} and faster than the \glsxtrshort{DP}~\eqref{eq:Dormand-Prince_Tableu}. \par
...
Given the backstepping formulation~\ref{sub:backstepping}, it is assumed the previous obtained \glsxtrshort{CLF}~\eqref{eq:New_2-CLF-S} as the first-order formulation and \eqref{eq:2-CLF-S_Constraint} as the condition.\par 
Now, considering a second-order system \eqref{eq:HH-NL-System}, the \glsxtrshort{CLF} that are trying to converge to \(\bar{\xVec}_C\) and  \(\bar{\xVec}_N\) respectively are:
...
Given \(V_C\) and \(V_N\) it is possible to obtain the new \glsxtrshort{CLF} \(V\) equally as \eqref{eq:1stPhase_2-CLF-S}-\eqref{eq:New_2-CLF-S} but keeping the first-order sigmoide function \(\sigma(\xVec)\) from \eqref{eq:sig_CLF_Vc0}. That beeing said, it is possible to reduce the \glsxtrshort{CLF} \(V\) equation to:
...
Regarding the stability objective changing condition, it maintains equal to \eqref{eq:New_2-CLF-S_actuationPhaseSet} using the first-order \glsxtrshort{CLF} and control input \(\mathbf{k}_0\):
...
Finally, with \glsxtrshort{A-JO}~\ref{subsec:Just_Optimized_Algorithm}, using the cost function \eqref{eq:Cost_Function_Alpha_last_state} adapt the \glsxtrshort{CBF} slope \(\alpha_d\) to the new low and high order \glsxtrshort{CLF}s and improve possibly even more the state convergence to the final equilibrium point.\\
...
The transitive (\(i = i+1\)) condition is based on the first-order \glsxtrshort{CLF} \(V_{C_i}\) as it was already seen~\eqref{eq:HH_New_2-CLF-S_actuationPhaseSet} given by:
...
\end{equation}\\
  ...
  \begin{algorithm}
    ...
    $i \gets 0$\;
    \While{$i < nIte$, $i++$}{ 
        $[dir, \hspace{0.1em} \xVec_{int}] = NCDL(f, \xVec_0, N, \Delta t, V_{1st-waypoint}, h) \hspace{0.2em} \begin{bmatrix*}[l] decision^* \gets \text{\glsxtrshort{CLF-CBF}~\ref{sub:closed_form} } \\ integration^* \gets \text{\glsxtrshort{RK}4~\eqref{eq:RK4_Tableu}} \end{bmatrix*}$\;
    ...
    $\alpha_d \gets$ \glsxtrshort{A-JO} $\begin{bmatrix*}[l] g_{\xVec_0}(\alpha_d)^* \gets \text{Last State~\eqref{eq:Cost_Function_Alpha_last_state}}\end{bmatrix*}$\;
    \caption{CLF-Summed (\glsxtrshort{A-CLF-S})} \label{alg:A-CLF-S}
  \end{algorithm}

 Given the new first and high order \glsxtrshort{CLF}, its conditions and transitions, and the new \(\alpha_d\), the system control action are decided based on the \glsxtrshort{CLF-CBF} closed-form controller~\eqref{eq:closed-form_controller}. \\
...
From both equilibrium points is defined two \glsxtrshort{CLF}, \(V_C\) following the current equilibrium point \(\bar{\xVec_C}\) and \(V_N\) stabilizing in \(\bar{\xVec_N}\). If \(\zeta \to \infty\), according to equations \eqref{eq:1stPhase_2-CLF-S} and \eqref{eq:New_2-CLF-S}:
